  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser) {
        ...
        // 1. Đồng bộ với Backend SQLite để lấy JWT và Role
        try {
          const syncRes = await authApi.syncFirebase({
          ...
          // 2. Cập nhật Profile từ Backend (Ưu tiên Role từ SQLite)
          ...
          if (userDoc.exists()) {
            currentProfile = { 
              ...
              role: backendUser.role // Ghi đè role từ SQLite
            };
          } else {
            currentProfile = {
              uid: firebaseUser.uid,
